...
function applyLexiconTheme() {
    ...
    style.innerText = `
        ...
        .cell { background: var(--obsidian); box-shadow: ${m.effects.bevel}; position: relative; display: flex; align-items: center; justify-content: center; font-size: 0.6rem; font-weight: 900; color: rgba(255,255,255,0.1); }
        ...
        .dragging { z-index: 5000; pointer-events: none; opacity: 0.9; }
...
function buildUI() {
    ...
    ui.buttons.forEach(btn => {
        const b = document.createElement('button'); b.className = btn.class; b.innerText = btn.text;
        b.onclick = () => {
...
function initBag() { 
    bag = []; tiles.distribution.forEach(d => { for(let i=0; i<d.q; i++) bag.push({...d}); });
    bag.sort(() => Math.random() - 0.5); updateBagUI(); 
}
...
function makeDraggable(el) {
    ...
    const start = (e) => {
        ...
        activeTile = el;
        el.style.width = '44px'; el.style.height = '44px'; el.style.position = 'fixed';
    ...
    const end = (e) => {
        ...
        if (t.clientX >= gRect.left && t.clientX <= gRect.right && t.clientY >= gRect.top && t.clientY <= gRect.bottom) {
            ...
            if (targetCell && !targetCell.querySelector('.tile')) {
                targetCell.appendChild(el); el.style.position = 'absolute'; el.style.left = '0'; el.style.top = '0';
                el.style.width = '100%'; el.style.height = '100%'; el.style.transform = 'none';
                placedTiles.push({el, index: idx}); return;
            }
        }
        const rack = document.getElementById('rack'); rack.appendChild(el);
...
function swapTiles() {
    ...
    Array.from(rack.querySelectorAll('.tile')).forEach(t => {
        bag.push({ l: t.dataset.raw, v: parseInt(t.dataset.value) }); t.remove();
    });
    bag.sort(() => Math.random() - 0.5); refillRack('rack'); switchTurn('bot');
}
...
function recallTiles() {
    ...
    placedTiles.forEach(p => {
        p.el.classList.remove('on-board'); p.el.style.position = 'relative'; p.el.style.transform = 'none';
        r.appendChild(p.el);
    });
    placedTiles = [];
}
